...
#ifdef STOCKFISH_EMBEDDED
                // LOCAL PATCH: same reason as in position() - a failed set()
                // leaves a half-built board behind.
...
#ifdef STOCKFISH_EMBEDDED
    // LOCAL PATCH: a bad argument no longer ends the process, so the search has
    // to be abandoned here. Without this, `go movetime abc` would fall through
    // with the arguments parsed so far - i.e. an infinite search that only a
    // `stop` could end, which is worse for the app than doing nothing.
    if (criticalError)
        return;
#endif
...
u64 UCIEngine::perft(const Search::LimitsType& limits) {
    ...
    if (auto err = std::get_if<PositionSetError>(&result))
    {
        terminate_on_critical_error(err->what());
#ifdef STOCKFISH_EMBEDDED
        // LOCAL PATCH: terminate_on_critical_error returns here, and std::get<u64>
        // on the error alternative would abort a -fno-exceptions build.
        return 0;
#endif
    }
...
void UCIEngine::position(std::istringstream& is) {
    ...
    if (err.has_value())
    {
        terminate_on_critical_error(err->what());
#ifdef STOCKFISH_EMBEDDED
        // LOCAL PATCH: Position::set() zeroes the board before it validates, so
        // a rejected FEN leaves the engine on a half-built position - no kings,
        // say - and the next `go` would search it and crash. Upstream never gets
        // here. Put the engine on a position that is known to be legal; the
        // caller has been told the command failed and will send a new one.
        engine.set_position(StartFEN, {});
#endif
    }
}
...
void UCIEngine::terminate_on_critical_error(const std::string& message) {
    sync_cout << "info string CRITICAL ERROR: Command `" << currentCmd
              ...
              << sync_endl;
#ifndef STOCKFISH_EMBEDDED
    std::exit(1);
#else
    criticalError = true;
#endif
    // LOCAL PATCH (dawikk-stockfish): see uci.h. Embedded in the app's process,
    // exit() here would kill the app over one bad command, so the caller returns
    // to the UCI loop instead. The native module watches stdout for the CRITICAL
    // ERROR line and surfaces it to JS as an engine error event.
}
